...
<a aria-label="link" {href} class={cn('flex items-center gap-x-2 text-muted-foreground text-sm font-medium pl-6 transition-all hover:text-slate-600 hover:bg-muted', {
    'text-primary bg-primary/20 hover:bg-primary/20 hover:text-primary':isActive 
})}>
<div class="flex items-center gap-x-2 py-4">
    <svelte:component this={icon} class= {cn('text-muted-foreground', {'text-primary':isActive})}
    size={22}  />
